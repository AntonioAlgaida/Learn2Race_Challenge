...
class CustomReward(GranTurismo):
    ...
    :param fsm: bonus reward for being near the centerline
    ...
    def get_reward(self, state, oob_flag=False):
        ...
        steering = pose_data[STEERING_IDX]
        ...
        bonus = self._reward_centerline(race_idx, loc, progress_reward, steering)
        smooth_steering =  self._reward_smooth_steering(steering)
        return oob_reward + progress_reward + bonus + smooth_steering

    def _reward_centerline(self, race_idx, loc, progress_reward, steering):
        ...
        return 1/(dist/0.1) * 1/(1+np.abs(steering))
        # if dist < self.d_thresh*0.1:
        #     return self.c_bonus*10
        # elif dist < self.d_thresh*0.5:
        #     return self.c_bonus*5
        # elif dist < self.d_thresh:
        #     return self.c_bonus
        # else:
        #     return 0.0


    def _reward_smooth_steering(self, steering):
        return -np.abs(steering)**2.5*1000
        # abs_steering = np.abs(steering - self.prev_steering)
        # self.prev_steering = steering
        # return -abs_steering*50
