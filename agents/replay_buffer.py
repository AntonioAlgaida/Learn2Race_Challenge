...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size):
        self.obs_buf = np.zeros((size, 4, obs_dim), dtype=np.float32)  # +1:spd #core.combined_shape(size, obs_dim)
        self.obs2_buf = np.zeros((size, 4, obs_dim), dtype=np.float32) #np.zeros((size, obs_dim), dtype=np.float32)  # +1:spd #core.combined_shape(size, obs_dim)
